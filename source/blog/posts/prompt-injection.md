...
Prompt injection is a fundamental challenge in LLM application security, and it does not have a complete solution. The defenses described here — input validation, privilege separation, output validation, and minimal permissions — raise the bar significantly without guaranteeing perfect protection.

For applications that process untrusted content (documents, web pages, emails, user-generated data), indirect injection is the primary risk. Design with this in mind from the start: separate trusted system context from untrusted user data, validate outputs against expected schemas, and constrain what agent actions are possible even if injection succeeds.
...
Log it with full context, return a safe default response to the user, and alert your security team if the pattern is novel. Aggregate injection attempt data to identify emerging attack patterns and improve your defenses.
